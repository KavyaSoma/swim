...
<form class="form-horizontal" style="background:#fff;" method="post" action="{{url('manageparticipants/'.$event_id.'/'.$heat_id) }}">
  {{csrf_field()}}
<div class="form-group">
<label class="control-label col-sm-4 form_heat" for="sel1">Select Heat:</label>
<div class="col-sm-6">
  @if(count($heats)>0)
<select class="form-control country" id="sel1"  name="heats_id" onchange="heats('{{url('manageparticipants/'.$event_id)}}')">
...
<form method="post" action="{{url('manageparticipants/'.$event_id.'/'.$heat_id) }}">
	{{csrf_field()}}
